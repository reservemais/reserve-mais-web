...
import { useModalStore, usePaginationStore, useUserStore } from "@/store";
...
const UserFilter = () => {
  ...
  const { fetchData, selectedFilter, hasFilteredUsers } = useUserStore();
  const { pagination } = usePaginationStore();
  const [filterData, setFilterData] = useState(
    selectedFilter ||
      ({
        type: "",
      } as Pick<userApiModel, "type">)
  );

  const { query, queryWithFilters } = userQueries(pagination, filterData);
  const { userFilterTypeOptions } = D.filtersOptions();
